        ...
        <div class="content cate-list">
            ...
            <div class="cate-header">
                ...
                <h1>여행지 / 축제</h1>
                 ...
                 <button class="write_btn" style="font-size:11px" @click="write_trip">여행지/축제 등록</button>
           ...
           <div class="s1" v-for="(row, rowIndex) in rowList" :key="rowIndex" style="height: 270px;  scale:120%">
                <div class="card" style="width: 230px; height: 370px; border:none; margin: 0 1%"
                    ...
                    <a class="imgSpace" :href="'http://localhost:8080/' + (item.trip_no ? 'tvdetail/' + item.trip_no : 'fsdetail/' + item.trip_no)">
                        <img :width="230"
                    ...
                    <div class="card-body" style="padding: 1px;">
                        ...
                        <!-- <td><button class="btn btn-light" @click="goToUpdateGoods(item.tv_no ? item.tv_no : item.fs_no)">수정</button></td> -->
                       <td><button class="btn btn-outline-danger" style="scale:80%" @click="deleteGoods(item.trip_no)">여행 삭제</button></td>
                         <!-- <td><button class="btn btn-light" @click="goToUpdate2Goods(item.tv_no ? item.tv_no : item.fs_no)">수정</button></td> -->
...
export default {
    ...
    mounted() {
        // this.getTvList(0);
        // this.getFsList(0);
        this.getAllList(0); //최신순이 처음 실행할때 나오도록
        // this.getAllList2(0);
    },
    ...
    methods: {
         ...
         deleteGoods(tripno) {
            this.$swal({
                ...
                if (result.value) {
                    axios({
                        url: `http://localhost:3000/admin/delete_goods`,
                        method: "POST",
                        data: {
                        ...
                        if (res.data.message == 'delete_complete') {
                            ...
                            window.location.href = `http://localhost:8080/admin/admintrip`;
                        }
                    }).catch(() => {
                        this.$swal("오류 발생");
                    });
        ...
        //         if (result.value) {
        //             axios({
        //                 url: `http://localhost:3000/admin/delete2_goods`,
        //                 method: "POST",
        //                 data: {
        ...
        async getAllList(sortCase) {
            try {
                const response = await axios.get(`http://localhost:3000/admin/all/${sortCase}`);
        ...
        async getAllList2(sortCase) {
            try {
                const response = await axios.get(`http://localhost:3000/admin/all2/${sortCase}`);
