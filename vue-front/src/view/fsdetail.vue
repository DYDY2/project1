...
export default {
  ...
  methods: {
      ...
      if (this.reviewText.trim() !== "" && this.selectedStars > 0) {
        ...
        try {
          ...
          const response = await axios.post(
            `http://localhost:3000/trip/festivals/${trip_no}/reviews`,
            newReview
          );
    ...
    async fetchFestival(trip_no) {
      try {
        console.log(`Fetching festival with fs_no: ${trip_no}`);
        const response = await axios.get(
          `http://localhost:3000/trip/festivals/${trip_no}`
        );
    ...
    async likeCheck() {
      ...
      const response = await axios.post(
        `http://localhost:3000/trip/likeCheck2/${this.$route.params.trip_no}/${user_no}`
      );
    ...
    async likeInsert() {
      if (this.user.user_no === "") {
        ...
        await axios.post(
          `http://localhost:3000/trip/likeInsert2/${this.$route.params.trip_no}/${this.user.user_no}`
        );
        this.isLiked = true;
        await this.getLike();
      }
    ...
    async likeDelete() {
      if (this.user.user_no === "") {
        ...
        await axios.post(
          `http://localhost:3000/trip/likeDelete2/${this.$route.params.trip_no}/${this.user.user_no}`
        );
        this.isLiked = false;
        await this.getLike();
      }
    ...
    async getLike() {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3000/trip/likeCount2/${trip_no}`
        );
