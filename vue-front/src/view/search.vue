            ...
            <div class="s1" v-for="(row, rowIndex) in rowList" :key="rowIndex" style="height: 270px;  scale:120%">
                <div class="card" style="width: 230px; height: 370px; border:none; margin: 0 1%"
                    ...
                    <a class="imgSpace" :href="'http://localhost:8080/' + (item.trip_no ? 'tvdetail/' + item.trip_no : 'fsdetail/' + item.trip_no)">
                        <img :width="230"
                            :src="item.trip_img ? require(`../../../node-back/uploads/${item.trip_img}`) : (item.fs_img ? require(`../../../node-back/uploads/${item.trip_img}`) : '/empty.jpg')"
                            alt="이미지" />
                    </a>
...
export default {
    ...
    methods: {
        ...
        async getAllList(sortCase) {
            try {
                const response = await axios.get(`http://localhost:3000/best/allSearch/${this.keyword}/${sortCase}`);
         ...
         async getAllList2(sortCase) {
            try {
                const response = await axios.get(`http://localhost:3000/best/all2Search/${this.keyword}/${sortCase}`);
                // const response = await axios.get(`http://localhost:3000/best/tvSearch/${this.keyword}/${sortCase}`);
